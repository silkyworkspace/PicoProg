...
class TestSettings(unittest.TestCase):
    ...
    def _get_user(self, email):
        conn = sqlite3.connect(TEST_DB_PATH)
        row = conn.execute(
            'SELECT username, profile FROM users WHERE email = ?', (email,)
        ).fetchone()
        conn.close()
        return row

    def test_settings_requires_login(self):
        # 未ログインで /settings → ログインページへリダイレクト
    ...
    def test_update_username(self):
        ...
        user = self._get_user('alice@example.com')
        self.assertEqual(user[0], 'alice_new')
    ...
    def test_update_profile(self):
        ...
        user = self._get_user('alice@example.com')
        self.assertEqual(user[1], 'Flaskを勉強中です')
    ...
    def test_empty_username_is_rejected(self):
        ...
        self.assertIn('ユーザー名を入力してください', response.data.decode('utf-8'))

    def test_profile_too_long_is_rejected(self):
        # プロフィールが200文字を超える場合はエラーになる
        self._login('alice@example.com')
        long_profile = 'あ' * 201
        response = self.client.post('/settings', data={
            'username': 'alice',
        ...
        self.assertIn('プロフィールは200文字以内', response.data.decode('utf-8'))
